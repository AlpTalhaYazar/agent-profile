...
export function ProfileMcpToolsPanel({
  ...
  }, [formValidation, hasSelection, onPreviewStateChange, onValidationStateChange, open, target.status]);
  ...
  const handleAddGitHub = React.useCallback(() => {
    updateDraft((current) => ({ ...current, tools: [...current.tools, createGitHubProfileMcpToolDraft()] }));
  }, [updateDraft]);

  const handleAddBlank = React.useCallback(() => {
    updateDraft((current) => ({ ...current, tools: [...current.tools, createDefaultProfileMcpToolDraft({ name: uniqueDraftToolName(current.tools) })] }));
  }, [updateDraft]);
  ...
  const handlePreview = React.useCallback(async () => {
    ...
    if (!profileApi?.validate || !profileApi.preview) {
      ...
      setPreviewItems(createSafeProfileMcpToolsPreviewSummary(target.status === "writable" ? target.content : null, patch.content));
    ...
    try {
      ...
      const validationIssues = sanitizeValidationIssues(normalizeValidationIssues(validationResult));
      ...
      const summary = createSafeProfileMcpToolsPreviewSummary(target.content, patch.content);
      ...
      if (preview.status === "error") {
        const message = "Profile Tools preview could not be prepared. Review the fields and try again.";
        setPreviewStatus("error");
        setPreviewError(message);
        onPreviewStateChange({ status: "error", effective: null, diff: [], errorMessage: message });
      ...
      announce(summary.length > 0 ? "Profile Tools preview ready" : "Profile Tools preview has no changes");
    } catch (error) {
  ...
  }, [announce, currentEffective, cwd, draft, onPreviewStateChange, onValidationStateChange, selectedAuthId, selectedRole, target]);
  ...
  const targetTone = target.status === "writable" ? "success" : target.status === "invalid" ? "danger" : "warning";
  const previewTone = previewStatus === "ready" ? (previewItems.length > 0 ? "info" : "success") : previewStatus === "error" ? "danger" : previewStatus === "loading" ? "warning" : "neutral";
  const targetMessage = target.status === "writable" ? "Guided Tools will update MCP servers on the selected Agent Profile only." : target.message;
    ...
    <>
        ...
        <form className="flex max-h-[92vh] min-h-0 flex-col" onSubmit={handleSave}>
            ...
            <div className="flex min-w-0 items-start justify-between gap-4">
              <div className="flex min-w-0 gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-tertiary">Guided Tools</p>
                  <h2 className="mt-1 text-xl font-semibold tracking-[-0.02em] text-primary" id="profile-tools-title">
                    Add MCP tools to {profile.name}
                  </h2>
                  <p className="mt-1 text-sm leading-6 text-secondary">
                    Configure MCP servers with logical secret names. Secret values stay in Auth and never appear here.
                  </p>
                </div>
              </div>
              <Button aria-label="Close Profile Tools" className="min-h-10 min-w-10 shrink-0" onClick={() => requestLeave(null)} type="button" variant="ghost">
          ...
          <div className="app-scrollbar grid min-h-0 flex-1 gap-0 overflow-auto lg:grid-cols-[minmax(0,1.35fr)_minmax(20rem,0.85fr)]">
            <div className="grid gap-5 px-6 py-5">
              <section className="rounded-xl border border-default bg-canvas/60 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <StatusChip tone={targetTone}>{target.status === "writable" ? "Ready to edit" : "Needs advanced edit"}</StatusChip>
                </div>
                {target.status !== "writable" ? (
                  <div className="mt-4">
                    <Button data-testid="profile-tools-open-advanced" onClick={onOpenAdvanced} type="button" variant="secondary">
              ...
              <section className="rounded-xl border border-default bg-surface p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button disabled={target.status !== "writable" || isSaving} onClick={handleAddGitHub} ref={initialFocusRef} type="button" variant="secondary" data-testid="profile-tools-add-github">
                      <Sparkles className="h-4 w-4" aria-hidden="true" />
                      Add GitHub tool
                    </Button>
                    <Button disabled={target.status !== "writable" || isSaving} onClick={handleAddBlank} type="button" variant="secondary" data-testid="profile-tools-add-blank">
                ...
                {draft.tools.length > 0 ? (
                  <div className="mt-4 grid gap-4" data-testid="profile-tools-list">
                    {draft.tools.map((tool, index) => (
                      <ToolDraftCard
                        ...
                        key={tool.id}
                        onChange={(nextTool) => updateDraft((current) => ({ ...current, tools: current.tools.map((candidate) => candidate.id === tool.id ? nextTool : candidate) }))}
                        onDelete={() => updateDraft((current) => ({ ...current, tools: current.tools.filter((candidate) => candidate.id !== tool.id) }))}
                        tool={tool}
                        toolIndex={index}
                      />
                    ))}
                  ...
                  <div className="mt-4 rounded-lg border border-dashed border-subtle bg-canvas/60 px-4 py-8 text-sm text-secondary" data-testid="profile-tools-empty">
                    No profile-owned MCP tools yet. Add a GitHub tool to start with a logical secret named github.pat.
                  </div>
                )}
              </section>
            </div>
            ...
            <aside className="border-t border-subtle bg-canvas/65 px-6 py-5 lg:border-l lg:border-t-0">
              <section aria-live="polite" className="sticky top-0 grid gap-4" data-testid="profile-tools-preview">
                <div className="rounded-xl border border-default bg-surface p-4 shadow-xs">
                  <div className="flex items-start gap-3">
                    <IconFrame icon={previewStatus === "error" ? AlertTriangle : Eye} size="sm" tone={previewStatus === "error" ? "danger" : "accent"} />
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="text-sm font-semibold text-primary">Preview impact</h3>
                        <StatusChip tone={previewTone}>{formatPreviewStatus(previewStatus)}</StatusChip>
                      </div>
                  ...
                  {previewItems.length > 0 ? (
                    <ul className="mt-4 grid gap-2">
                      {previewItems.slice(0, 12).map((item, index) => (
                        <li className="rounded-md border border-subtle bg-canvas/70 px-3 py-2 text-sm text-secondary" key={`${item.name}:${item.change}:${index}`}>
                          <span className="font-medium text-primary">{formatPreviewChange(item.change)}</span> MCP tool · {item.name} · {item.transport} · {item.detail}
                        </li>
                      ))}
                    </ul>
                  ) : null}
                ...
                <div className="rounded-xl border border-default bg-surface p-4">
                  ...
                  <ul className="mt-3 grid gap-2 text-sm text-secondary">
                    <ReadinessLine ok={target.status === "writable"}>Writable profile target</ReadinessLine>
                    <ReadinessLine ok={hasSelection}>Selected profile still available</ReadinessLine>
                    <ReadinessLine ok={!issuesByField.name}>Unique safe server names</ReadinessLine>
                    <ReadinessLine ok={!issuesByField.commandOrUrl && !issuesByField.transport}>Valid transport endpoint</ReadinessLine>
                    <ReadinessLine ok={!issuesByField.env && !issuesByField.headers}>Logical secret names only</ReadinessLine>
                  </ul>
                </div>

                {issues.length > 0 || saveError || !hasSelection || target.status !== "writable" ? (
                  <div className="rounded-xl border border-status-danger bg-status-danger-soft p-4 text-sm text-status-danger" data-testid="profile-tools-error" role="alert">
                    <p className="font-semibold">Tools needs attention</p>
                    <ul className="mt-2 grid gap-1">
                      {target.status !== "writable" ? <li>{target.message}</li> : null}
                      {!hasSelection ? <li>Choose a valid Agent Profile before saving tools.</li> : null}
                      {issues.slice(0, 5).map((issue, index) => <li key={`${issue.path}:${index}`}>{issue.message}</li>)}
          ...
          <div className="flex flex-row flex-wrap justify-end gap-2 border-t border-subtle bg-surface/95 px-6 py-4">
            <Button data-testid="profile-tools-cancel" disabled={isSaving} onClick={() => requestLeave(null)} ref={cancelButtonRef} type="button" variant="secondary">Cancel</Button>
            <Button data-testid="profile-tools-preview-action" disabled={target.status !== "writable" || isSaving || !hasSelection || formValidation.issues.length > 0} onClick={() => void handlePreview()} type="button" variant="secondary">
              <Eye className="h-4 w-4" aria-hidden="true" />
              Preview Tools
            </Button>
            <Button data-testid="profile-tools-save" disabled={saveDisabled} type="submit" variant="primary">
      ...
      {dirtyPromptOpen ? (
        <>
          ...
          <dialog aria-labelledby="profile-tools-dirty-title" className="fixed left-1/2 top-1/2 z-[61] grid w-full max-w-md -translate-x-1/2 -translate-y-1/2 gap-4 rounded-md border border-border bg-popover p-6 text-popover-foreground shadow-lg" data-testid="profile-tools-dirty-dialog" onCancel={(event: React.SyntheticEvent<HTMLDialogElement>) => { event.preventDefault(); cancelDirtyPrompt(); }} open>
            <header className="flex flex-col gap-1.5">
              <h2 className="text-base font-semibold text-foreground" id="profile-tools-dirty-title">Save Profile Tools changes?</h2>
              <p className="text-sm text-muted-foreground">You have unsaved guided Tools edits. Save before leaving, discard the draft, or stay here to keep editing.</p>
            </header>
            {saveDisabledReason ? <p className="rounded-md border border-status-warning bg-status-warning-soft px-3 py-2 text-sm text-status-warning">{saveDisabledReason}</p> : null}
            {saveError ? <p className="rounded-md border border-status-danger bg-status-danger-soft px-3 py-2 text-sm text-status-danger" role="alert">{saveError}</p> : null}
            <div className="flex flex-row flex-wrap justify-end gap-2">
              <Button data-testid="profile-tools-dirty-cancel" disabled={isSaving} onClick={cancelDirtyPrompt} type="button" variant="ghost">Keep editing</Button>
              <Button data-testid="profile-tools-dirty-discard" disabled={isSaving} onClick={discardDirtyPromptAndContinue} type="button" variant="secondary">Discard changes</Button>
              <Button data-testid="profile-tools-dirty-save" disabled={!canSaveTools || isSaving} onClick={() => void saveDirtyPromptAndContinue()} type="button" variant="primary">{isSaving ? "Saving…" : "Save Tools"}</Button>
            </div>
          </dialog>
        </>
      ) : null}
...
function ToolDraftCard({
  ...
  return (
    <section className="rounded-lg border border-subtle bg-canvas/60 p-4" data-testid="profile-tools-tool-card">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h4 className="text-sm font-semibold text-primary">MCP tool {toolIndex + 1}</h4>
          <p className="mt-1 text-sm text-secondary">Use logical secret names such as github.pat; do not paste token values.</p>
        </div>
        <Button aria-label={`Delete MCP tool ${toolIndex + 1}`} className="min-h-10" disabled={disabled} onClick={onDelete} type="button" variant="ghost" data-testid="profile-tools-delete-tool">
      ...
      {tool.hiddenAdvancedFieldCount > 0 ? (
        <p className="mt-3 rounded-md border border-status-warning bg-status-warning-soft px-3 py-2 text-sm text-status-warning">
          {tool.hiddenAdvancedFieldCount} advanced value{tool.hiddenAdvancedFieldCount === 1 ? "" : "s"} hidden. Open Profile Workspace before saving this tool.
        </p>
      ) : null}
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,0.7fr)_11rem]">
        <Field label="Server name" htmlFor={`${tool.id}-name`} {...fieldErrorProps(issuesByField.name)}>
          <Input disabled={disabled} id={`${tool.id}-name`} onChange={(event) => onChange({ ...tool, name: event.currentTarget.value })} value={tool.name} data-testid="profile-tools-server-name" />
        </Field>
        <Field label="Transport" {...fieldErrorProps(issuesByField.transport)}>
          <select aria-label={`MCP tool ${toolIndex + 1} transport`} className="min-h-10 rounded-md border border-default bg-canvas px-3 text-sm text-primary shadow-xs focus:outline-none focus:ring-2 focus:ring-focus disabled:cursor-not-allowed disabled:opacity-50" disabled={disabled} onChange={(event) => onChange({ ...tool, transport: event.currentTarget.value as TransportType })} value={tool.transport} data-testid="profile-tools-transport">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <Field label={isRemote ? "URL" : "Command"} htmlFor={`${tool.id}-command-or-url`} description={isRemote ? "Use an http or https MCP endpoint." : "Use the command name, not a token or secret reference."} {...fieldErrorProps(issuesByField.commandOrUrl)}>
          <Input className={isRemote ? "" : "font-mono text-xs"} disabled={disabled} id={`${tool.id}-command-or-url`} onChange={(event) => onChange({ ...tool, commandOrUrl: event.currentTarget.value })} placeholder={isRemote ? "https://example.test/mcp" : "npx"} value={tool.commandOrUrl} data-testid="profile-tools-command-url" />
        </Field>
        <Field label="Args" htmlFor={`${tool.id}-args`} description="One argument per line. Keep credentials in logical secrets." {...fieldErrorProps(issuesByField.args)}>
          <textarea className="min-h-24 rounded-md border border-default bg-canvas px-3 py-2 font-mono text-xs text-primary shadow-xs focus:outline-none focus:ring-2 focus:ring-focus disabled:cursor-not-allowed disabled:opacity-50" disabled={disabled || isRemote} id={`${tool.id}-args`} onChange={(event) => onChange({ ...tool, argsText: event.currentTarget.value })} value={tool.argsText} data-testid="profile-tools-args" />
        </Field>
      </div>
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <SecretRowsEditor disabled={disabled} error={issuesByField.env} label="Environment logical secrets" onChange={(envRows) => onChange({ ...tool, envRows })} rows={tool.envRows} testId="profile-tools-env-rows" />
        <SecretRowsEditor disabled={disabled || !isRemote} error={issuesByField.headers} label="Header logical secrets" onChange={(headerRows) => onChange({ ...tool, headerRows })} rows={tool.headerRows} testId="profile-tools-header-rows" />
      </div>
    </section>
  );
}
    ...
    <section className="rounded-lg border border-subtle bg-surface p-3" data-testid={testId}>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-secondary">The value is a logical Auth secret name, not the secret itself.</p>
        </div>
        <Button disabled={disabled} onClick={() => onChange([...rows, { id: createLocalId(), key: "", secretName: "" }])} type="button" variant="secondary">Add</Button>
      </div>
      {error ? <p className="mt-2 rounded-md border border-status-danger bg-status-danger-soft px-3 py-2 text-xs text-status-danger">{error}</p> : null}
      {rows.length > 0 ? (
        <div className="mt-3 grid gap-2">
          {rows.map((row, index) => (
            <div className="grid gap-2 sm:grid-cols-[minmax(0,0.85fr)_minmax(0,1fr)_auto]" key={row.id}>
              <Input aria-label={`${label} ${index + 1} field`} disabled={disabled} onChange={(event) => onChange(rows.map((candidate) => candidate.id === row.id ? { ...candidate, key: event.currentTarget.value } : candidate))} placeholder="Authorization" value={row.key} />
              <Input aria-label={`${label} ${index + 1} secret name`} disabled={disabled} onChange={(event) => onChange(rows.map((candidate) => candidate.id === row.id ? { ...candidate, secretName: event.currentTarget.value } : candidate))} placeholder="github.pat" value={row.secretName} />
              <Button aria-label={`Remove ${label} ${index + 1}`} className="min-h-10" disabled={disabled} onClick={() => onChange(rows.filter((candidate) => candidate.id !== row.id))} type="button" variant="ghost">Remove</Button>
            </div>
          ))}
        </div>
...
function mapToolsIssuesByField(issues: readonly { field: ProfileMcpToolsValidationIssue["field"]; message: string }[]): Partial<Record<ProfileMcpToolsValidationIssue["field"], string>> {
...
function sanitizeValidationIssues(issues: readonly ValidationIssue[]): ValidationIssue[] {
  return issues.map((issue, index) => ({ path: safeIssuePath(issue.path || `profile-tools.${index + 1}`), severity: issue.severity || "error", message: safeValidationMessage(issue) }));
}

function safeIssuePath(path: string): string {
  if (/keyring:\/\/|\$\{secret:|secretRef|bearer\s+|token|authorization/i.test(path)) return "profile-tools";
  return path || "profile-tools";
}

function safeValidationMessage(issue: ValidationIssue): string {
  if (/mcp|server|tool/i.test(issue.path)) return "Profile Tools needs a safe MCP server value before saving.";
  return "Profile Tools needs a safe value before saving.";
}

function createSafeMcpToolsEffectiveDiff(current: EffectiveConfig | null, preview: EffectiveConfig | null): DiffItem[] {
  if (!current || !preview) return [];
  const items: DiffItem[] = [];
  for (const key of Array.from(new Set([...Object.keys(current.mcpServers), ...Object.keys(preview.mcpServers)])).sort()) {
    ...
    items.push({ section: "mcpServers", key: safeDiffKey(key), change: before === undefined ? "added" : after === undefined ? "removed" : "changed" });
  }
  return items;
}
...
function getProfileMcpToolsSaveDisabledReason(input: { targetStatus: ProfileMcpToolsTarget["status"]; hasBlockingIssues: boolean; hasSelection: boolean; isSaving: boolean; previewStatus: ToolsAsyncStatus; }): string | null {
  ...
  if (input.previewStatus === "loading") return "Wait for Profile Tools preview to finish checking.";
  if (input.isSaving) return "Profile Tools is saving.";
  return null;
}

function fieldErrorProps(error: string | null | undefined): { error: string } | Record<string, never> {
  return error ? { error } : {};
}
...
function ReadinessLine({ children, ok }: { children: React.ReactNode; ok: boolean }): React.ReactElement {
  return (
    <li className="flex items-center gap-2">
      {ok ? <CheckCircle2 className="h-4 w-4 text-status-success" aria-hidden="true" /> : <AlertTriangle className="h-4 w-4 text-status-warning" aria-hidden="true" />}
