 ...
 * persisted to `~/.myclaude/ipc-cookie` with mode `0600`. The CLI reads it on
 * connect and sends it as the first message of the handshake; the daemon
 * compares against its in-memory copy. See `docs/06-security.md` "IPC
 * authentication" for the full triple-gate (filesystem perms + euid +
 * cookie) rationale.
 *
...
/** Mode for the parent directory (`~/.myclaude/`); user-only access. */
...
/**
 * Compute the absolute path to the cookie file for a given home directory.
 *
 * @param home - Home directory; defaults to `os.homedir()`.
 * @returns `<home>/.myclaude/ipc-cookie`.
 */
export function cookiePath(home: string = homedir()): string {
  return join(home, ".myclaude", "ipc-cookie");
}
 ...
 * Steps, in order:
 *
 *  1. `mkdir(parent, recursive: true, mode: 0o700)` — ensures the directory
 *     exists with strict perms even on first run.
 ...
 */
export async function writeBootCookie(home: string = homedir()): Promise<string> {
  const path = cookiePath(home);
  const dir = join(home, ".myclaude");
  await mkdir(dir, { recursive: true, mode: COOKIE_DIR_MODE });
 ...
 */
export async function readCookie(home: string = homedir()): Promise<string> {
  const path = cookiePath(home);
